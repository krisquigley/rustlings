...
impl Package {
    fn new(sender_country: String, recipient_country: String, weight_in_grams: i32) -> Package {
        if weight_in_grams <= 0 {
            panic!("You must define a weight higher than 0");
        } else {
    ...
    fn get_fees(&self, cents_per_grams: i32) -> i32 {
        self.weight_in_grams * cents_per_grams
    }
}
...
mod tests {
    ...
    fn calculate_transport_fees() {
        ...
        let cents_per_grams = 3;
        
        let package = Package::new(sender_country, recipient_country, 1500);
        
        assert_eq!(package.get_fees(cents_per_grams), 4500);
    }
}
